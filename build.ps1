...
try {
    ...
    Write-Host "     Versão portátil (.zip) criada em: $ZipPath" -ForegroundColor Green

    # 6b. Criar executável portátil Single-File (.exe único) em Releases
    Write-Host "     Criando executável portátil Single-File em Releases..." -ForegroundColor Yellow
    $SingleFileTempDir = Join-Path $RootDir "temp_singlefile"
    if (Test-Path $SingleFileTempDir) { Remove-Item -Path $SingleFileTempDir -Recurse -Force }

    Exec-Step "Publish-SingleFile" "dotnet" @(
        "publish", "src/RotinaRemote.Client/RotinaRemote.Client.csproj",
        "-c", $Configuration,
        "-r", "win-x64",
        "--self-contained", "true",
    ...
    $SingleFileSource = Join-Path $SingleFileTempDir "RotinaRemote.exe"
    $SingleFileDest = Join-Path $ReleasesDir "RotinaRemote-Portable.exe"
    $SingleFileDestAlt = Join-Path $ReleasesDir "RotinaRemote-Portable-SingleFile.exe"

    if (Test-Path $SingleFileSource) {
        Copy-Item -Path $SingleFileSource -Destination $SingleFileDest -Force
        Copy-Item -Path $SingleFileSource -Destination $SingleFileDestAlt -Force
        Write-Host "     Executável Single-File Portable criado em: $SingleFileDest" -ForegroundColor Green
        [void]$LogBuilder.AppendLine("SingleFile EXE:")
        [void]$LogBuilder.AppendLine($SingleFileDest)
    } else {
        Write-Host "     AVISO: Executável Single-File não foi encontrado em $SingleFileSource" -ForegroundColor Red
    }
